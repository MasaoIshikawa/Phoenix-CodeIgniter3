...
class Contacts extends CI_Controller {
	...
	function index($fields='date', $order_type='desc')
	{
	...
	{
		$result=$this->contacts_model->getContactById($contact_id);
		
		if($result['type']=='survy') {
			
			$this->data['surveydata']=$this->contacts_model->getsurvyById($result['contact_id']);
			
			}
		
		if($result['type']=='catalog') {
			
			$this->data['catalog']=$this->contacts_model->getCatalogById($result['contact_id']);
		...
		$this->contacts_model->setIsRead($contact_id);	
...
/*
	...
	*/
	function history($contact_id, $fields='', $order_type='asc'){
		...
		if($this->contacts_model->reply($contact_id)){ 
			...
			if ($this->email->send()){
				...
				$this->session->set_flashdata('message', 'Error while sending!');
           		...
           		redirect('admin/contacts/reply/'.$contact_id);
			}
