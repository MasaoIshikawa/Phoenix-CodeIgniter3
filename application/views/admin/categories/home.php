...
<h1 class="pull-left">Categories Controller</h1>
<a href="javascript:void(0);" onClick="$('#addform').show();" class="link-btn green pull-right">Add New</a>
<a href="<?php echo base_url('admin/categories/subcats') ?>"  class="link-btn green pull-right">Sub Categories Controller</a>
</div>	<!--/page title -->
...
<form id="catform" method="post" action="<?php echo site_url('admin/categories/save_categoy')?>"  > 
...
<form action="<?php echo site_url('admin/categories/update_categoy')?>" method="post" id="catform_<?php echo $cat['cat_id']; ?>"> 
<td><input type="text" name="title" value="<?php echo $cat['title']; ?>" class="form-control validate[required,custom[onlyLetterNumber]]">
<input type="hidden" name="cat_id" value="<?php echo $cat['cat_id']; ?>" class="form-control" style="width: 70%">
</td>
<td>
<textarea name="detail" cols="30" rows="5" class="form-control validate[required]"><?php echo $cat['description']; ?></textarea>
</td>
...
<img class="profileim_<?php echo $cat['cat_id']; ?>" style="width:35px; height:28px;" src="<?php echo base_url();?>images/admin/upload.png" /></a></td>

<td><input type="text"  name="status" value="<?php echo $cat['status']; ?>" class="form-control" ></td>
...
<img src="<?php echo base_url('images/admin'); ?>/remove-icon.png" alt="remove"></a>
  <div id="f<?php echo $cat['cat_id']; ?>" class="inline-popup" style="display:none;">
                <p>Are you sure you want to do this? <span>This action is not reversible</span><span> <button onClick="window.location='<?php echo base_url('admin/categories/delete_cat/'.$cat['cat_id']); ?>'" class="link-btn grey">Yes</button> <button onClick="$('#f<?php echo $cat['cat_id']; ?>').hide();" class="link-btn grey">No</button></span></p>
...
<script>
$('#catform').validationEngine('attach');
